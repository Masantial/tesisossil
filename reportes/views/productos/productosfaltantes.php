...
table th {background-color: #f37f21;color: black;}
</style>
</head>
<body>
<header>
...
p.Stock, p.StockMin, p.StockMin-p.Stock as CantCompra
FROM
producto p
WHERE
p.Stock < p.StockMin";
...
while ($mostrar = mysqli_fetch_array($result)) {
    ?>
            <tr>
                ...
                <td><?php echo $mostrar['StockMin'] ?></td>
                <td><?php echo $mostrar['CantCompra'] ?></td>
...
<script>
   $(document).ready(function(){
    var table = $('#articulosmenosdelmin').DataTable({
       orderCellsTop: true,
       fixedHeader: true,
       dom: 'Bfrtip',
    ...
    $('#articulosmenosdelmin thead tr').clone(true).appendTo( '#articulosmenosdelmin thead' );
    $('#articulosmenosdelmin thead tr:eq(1) th').each( function (i) {
