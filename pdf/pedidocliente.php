...
$id = $_GET['id'];
$sVenta = ejecutarSQL::consultar("SELECT * FROM venta v where v.NumPedido= $id ");
...
$pdf->Image('encabezado.png', 0, 2, -80);
...
while ($fila1 = mysqli_fetch_array($sDet, MYSQLI_ASSOC)) {
    ...
    $pdf->Cell(30, 10, utf8_decode('$' . number_format($fila1['PrecioProd'] * $fila1['CantidadProductos'],2)), 1, 0, 'C');
    $pdf->Ln(10);
    $suma += $fila1['PrecioProd'] * $fila1['CantidadProductos'];
    $bultos +=$fila1['CantidadProductos'];
