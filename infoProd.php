        ...
        <div class="container">
            <div class="row">
                <div class="page-header">
                    <h1>DETALLE DE PRODUCTO <small class="tittles-pages-logo">Ossil Envases</small></h1>
                </div>

                <?php
                $CodigoProducto = consultasSQL::clean_string($_GET['CodigoProd']);
                $productoinfo = ejecutarSQL::consultar("SELECT producto.CodigoProd,producto.NombreProd,producto.CodigoCat,categoria.Nombre,producto.marca,producto.Precio,producto.Descuento,producto.Stock,producto.Imagen FROM categoria INNER JOIN producto ON producto.CodigoCat=categoria.CodigoCat  WHERE CodigoProd='" . $CodigoProducto . "'");
                while ($fila = mysqli_fetch_array($productoinfo, MYSQLI_ASSOC)) {
                    echo '
                            ...
                            <div class="col-xs-12 col-sm-6">
                                ...
                                <br> <div>
                                <span style="color: green;"><b>En Stock</b></span>
                                
                            </div><br>
                                <h4><strong>Descripcion: </strong>' . $fila['NombreProd'] . '</h4><br>
                                <h4><strong>Categoria: </strong>' . $fila['Nombre'] . '</h4><br>
                                <h4><strong>Marca: </strong>' . $fila['marca'] . '</h4><br>
                                <h4><strong>Stock Disponible: </strong>' . $fila['Stock'] . '</h4><br>
                                <h1 align="Center"><strong></strong>$' . number_format(($fila['Precio'] - ($fila['Precio'] * ($fila['Descuento'] / 100))), 2, '.', '') . '</h1><br>';
                    if ($fila['Stock'] >= 1) {
                        if ($_SESSION['nombreAdmin'] != "" || $_SESSION['nombreUser'] != "") {
                            echo '<form action="process/carrito.php" method="POST" class="FormCatElec" data-form="">
                                            ...
                                            <label class="text-center"><big><b>Agrega la cantidad de productos que añadirás al carrito de compras (Maximo ' . $fila['Stock'] . ' productos)</b></big></label>
                                            <div class="form-group">
                                                <input type="number" class="form-control" value="1" min="1" max="' . $fila['Stock'] . '" name="cantidad" size="4">
                                            </div>
                    ...
                    echo '<br>
                                <a href="product.php" class="btn btn-lg btn-primary btn-raised btn-block"><i class="fa fa-mail-reply"></i>&nbsp;&nbsp;Regresar al Listado</a>
                            </div>
